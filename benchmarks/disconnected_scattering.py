...
if __name__ == "__main__":
    ...
    def test_integrand(x):
        ...
        val = S_disconnected(
            q_para, Eq, l_para, El, square_lattice, sigma_func_period=sigma_func_period
        ) * _gaussian_in_state(q_para, Eq, l_para, El)
        return abs(val) ** 2
    ...
    def test_integrand2(x):
        ...
        val = S_disconnected(
            q_para, Eq, l_para, El, square_lattice, sigma_func_period
        ) * _gaussian_in_state(q_para, Eq, l_para, El)
        val = abs(val) ** 2
        if is_scalar:
            return val[0]
