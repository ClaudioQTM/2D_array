...
if __name__ == "__main__":
    ...
    collective_lamb_shift = self_energy(ww
...
#    print(GH_filter_vectorized(205,np.array([0,0]), square_lattice))

    results = Parallel(n_jobs=8)(delayed(eigen_eq_itr_batch)(np.array([0,0]), 2*(square_lattice.omega_e + collective_lamb_shift), square_lattice, sigma_func_period_numba, np.exp(1j*phi),neval=int(5e6),tau_matrix_calculation=False) for phi in np.linspace(0, 2*np.pi, 8))
