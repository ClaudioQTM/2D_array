...
def _sample_for_k(
    ...
    l_batch = BZ_proj(Q_para - k_batch, lattice)
    ...
    for idx in valid_idx:
        ...
        values.extend(
            [
                t_reg(k_i, E1, lattice, sigma_func_period_numba)
                * t_reg(l_i, E - E1, lattice, sigma_func_period_numba)
                for E1 in E1_grid
            ]
        )
...
def disp_for_Q(E:float, Q_para:np.ndarray, n_k_samples:int, n_energy_points:int, sigma_func_period_numba, lattice,eps = 1e-3):
    ...
    samples = _sample_for_k(
        k_para_list, Q_para, E, eps, n_energy_points, sigma_func_period_numba, lattice
    )
...
if __name__ == "__main__":
    ...
    samples_array = Parallel(6, backend="loky")(
        delayed(disp_for_Q)(
            E,
            Q_para,
            n_samples=n_k_samples,
            sigma_func_period_numba=sigma_func_period_numba,
            lattice=square_lattice01,
        )
        for Q_para in Q_para_list
    )
