...
class EMField:
    ...
    def transverse_vector(k):
        if np.linalg.norm(k[:2]) == 0:
            ...
            e2 = (
                -1
                / (np.linalg.norm(k) * np.linalg.norm(k[:2]))
                * np.array([-k[2] * k[0], -k[2] * k[1], k[0] ** 2 + k[1] ** 2])
            )
            return e1, e2
    ...
    def make_helicity_polar_vec_builder():
        def polar_vec_builder(k, u=None):
            ...
            if np.any(nonzero_kxy):
                ...
                e2[nonzero_kxy] = (
                    -e2_num[nonzero_kxy]
                    / (k_abs[nonzero_kxy] * kxy_norm[nonzero_kxy])[:, None]
                )
    ...
    def GreenTensor_r(r, E):
        ...
        identity_mat = mp.matrix(identity_mat)
        rr = np.outer(r, r) / r_norm**2
        rr = mp.matrix(rr)

        return (
            mp.exp(mp.j * k * r_norm)
            / (4 * mp.pi * r_norm)
            * (A * identity_mat + B * rr)
        )
...
class SquareLattice:
    ...
    def __init__(
        ...
        self.d_norm = np.sqrt(
            float(3 * np.pi * epsilon_0 * hbar * c**3 * gamma / omega_e**3)
        )
    ...
    def g(self, u, v, k_xy, k_z):
        ...
        if k_xy.ndim == 2:
            ...
            if k_z.ndim == 0:
                ...
                if kz_vec.shape[0] != n:
                    raise ValueError(
                        "k_z must be scalar or have same length as k_xy batch."
                    )
    ...
    def ge(self, k):
        ...
        if k.ndim == 1:
            ...
            k_z = k[2]
            value = sum(
                sum(abs(self.g(u, v, k_xy, k_z)) ** 2 for u in (0, 1)) for v in (-1, 1)
            )
...
def real_space_summation(a, d, k_xy, omega):
    ...
    def summand(m, n, k_xy, omega):
        ...
        dGd = (
            mp.exp(mp.j * k * r_mn_norm)
            / (4 * mp.pi * r_mn_norm)
            * (A * d_norm**2 + B * drrd / r_mn_norm**2)
        )
        power = -mp.j * (m * k_xy[0] * a + n * k_xy[1] * a)
        return mp.exp(power) * dGd

    rsp_sum = mp.nsum(
        lambda m, n: 0 if (m == 0 and n == 0) else summand(m, n, k_xy, omega),
        [-mp.inf, mp.inf],
        [-mp.inf, mp.inf],
    )
    return rsp_sum
...
def k_space_summation(a, d, k_xy, omega, alpha):
    ...
    RegLatticeSum = (
        mp.j
        / (2 * a**2)
        * mp.nsum(lambda m, n: summand(m, n), [-mp.inf, mp.inf], [-mp.inf, mp.inf])
    )
...
def self_energy(k_x, k_y, a, d, omega, alpha):
    ...
    return complex(
        prefactor * k_space_summation(a, d, k_xy, omega, alpha)
        - 0.5j
        * omega**3
        * np.linalg.norm(d) ** 2
