...
start_point = 0.12
end_point = 0.999
self_energy_center_bz = Parallel(n_jobs=10)(
    delayed(self_energy)(0, 0, aa * 2 * np.pi, square_lattice.d, 1, 1, alpha)
    for aa in np.linspace(start_point, end_point, n_points)
)
...
ax1.set_title(
    "Real Part of Self-Energy at Center of 1st BZ", fontsize=14, fontweight="bold"
)
ax1.set_ylim(-3, 1)
ax1.set_xlim(0, 1)
...
ax2.set_title(
    "Imaginary Part of Self-Energy at Center of 1st BZ", fontsize=14, fontweight="bold"
)
...
print("=" * 60)
print(
    f"Real part - Min: {real_part.min():.6e}, Max: {real_part.max():.6e}, Mean: {real_part.mean():.6e}"
)
print(
    f"Imaginary part - Min: {imag_part.min():.6e}, Max: {imag_part.max():.6e}, Mean: {imag_part.mean():.6e}"
)
print(
    f"Magnitude - Min: {np.abs(self_energy_array).min():.6e}, Max: {np.abs(self_energy_array).max():.6e}"
)
print("=" * 60 + "\n")
