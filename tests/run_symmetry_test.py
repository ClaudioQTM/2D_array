...
def test_self_energy_reflection_symmetry_parallel(
    lattice, alpha=1e-4, tolerance=1e-10, n_tests=50, n_jobs=-1
):
    """
    ...
    print("Testing self-energy reflection symmetry in 1st BZ")
    ...
    results_parallel = Parallel(n_jobs=n_jobs, verbose=5)(
        delayed(compute_self_energy_point)(
            kx, ky, lattice.a, lattice.d, lattice.omega_e, alpha
        )
    ...
    for i in range(n_tests):
        ...
        print(
            f"         sigma(kx,-ky)  = {sigma_x_ref:.6e}  diff={diff_x:.2e} [{status_x}]"
        )
        print(
            f"         sigma(-kx,ky)  = {sigma_y_ref:.6e}  diff={diff_y:.2e} [{status_y}]"
        )

    print("\n" + "=" * 80)
    print("SUMMARY:")
    print("-" * 80)
    print("X-axis reflection (kx, ky) -> (kx, -ky):")
    print(
        f"  Passed: {results['x_reflection']['passed']}/{n_tests} ({100 * results['x_reflection']['passed'] / n_tests:.1f}%)"
    )
    ...
    print(
        f"  Passed: {results['y_reflection']['passed']}/{n_tests} ({100 * results['y_reflection']['passed'] / n_tests:.1f}%)"
    )
    ...
    if total_passed == total_tests:
        ...
        if results["failures"]:
            ...
            for f in results["failures"][:5]:
                print(
                    f"  {f['type']}: k=({f['kx']:.4f}, {f['ky']:.4f}), diff={f['diff']:.2e}"
                )
