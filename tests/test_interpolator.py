...
pytest.skip(
    "test_interpolator is a benchmark script, not a pytest unit test module",
...
def test_numba_vs_original(
    kx_grid, ky_grid, sigma_grid, lattice, n_random=1000, tolerance=1e-2, n_jobs=-1
):
    """
    ...
    sigma_original = create_self_energy_interpolator(
        kx_grid, ky_grid, sigma_grid, lattice, kx=1, ky=1
    )
    sigma_numba = create_self_energy_interpolator_numba(
        kx_grid, ky_grid, sigma_grid, lattice
    )
    ...
    for i, (kx, ky, desc) in enumerate(boundary_points):
        ...
        boundary_results["max_abs_diff"] = max(
            boundary_results["max_abs_diff"], abs_diff
        )
        boundary_results["max_rel_diff"] = max(
            boundary_results["max_rel_diff"], rel_diff
        )
    ...
    if boundary_results["failures"]:
        ...
        for f in boundary_results["failures"][:10]:  # Show first 10 failures
            ...
            print(
                f"      orig={f['orig']:.6e}, numba={f['numba']:.6e}, rel_diff={f['rel_diff']:.2e}"
            )
    ...
    for kx, ky in test_k_points:
        ...
        for Gx, Gy in G_vectors:
            ...
            val_shift_orig, val_shift_numba, _, _ = evaluate_point(
                kx_shifted, ky_shifted
            )

            # Check original periodicity
            diff_orig = abs(val_base_orig - val_shift_orig) / (
                abs(val_base_orig) + 1e-15
            )
            # Check numba periodicity
            diff_numba = abs(val_base_numba - val_shift_numba) / (
                abs(val_base_numba) + 1e-15
            )
                ...
                periodicity_results["failures"].append(
                    {
                        ...
                        "G": (Gx, Gy),
                        "diff_orig": diff_orig,
                        "diff_numba": diff_numba,
                    }
                )
                status = "FAIL"
                print(
                    f"  k=({kx:.3f},{ky:.3f}) + G=({Gx:.3f},{Gy:.3f}): "
    ...
    for res in parallel_results:
        random_results["max_abs_diff"] = max(
            random_results["max_abs_diff"], res["abs_diff"]
        )
        random_results["max_rel_diff"] = max(
            random_results["max_rel_diff"], res["rel_diff"]
        )
    ...
    total_passed = (
        ...
        + periodicity_results["passed"]
        + random_results["passed"]
    )
    total_tests = n_boundary + total_periodicity + n_random
    total_failed = (
        boundary_results["failed"]
        + periodicity_results["failed"]
        + random_results["failed"]
    )

    print(f"  Boundary tests:    {boundary_results['passed']}/{n_boundary} passed")
    print(
        f"  Periodicity tests: {periodicity_results['passed']}/{total_periodicity} passed"
    )
    print(f"  Random tests:      {random_results['passed']}/{n_random} passed")
    print("  ─────────────────────────────────")
    print(
        f"  TOTAL:             {total_passed}/{total_tests} passed ({100 * total_passed / total_tests:.1f}%)"
    )

    overall_max_rel = max(
        boundary_results["max_rel_diff"], random_results["max_rel_diff"]
    )
    overall_max_abs = max(
        boundary_results["max_abs_diff"], random_results["max_abs_diff"]
    )
...
def benchmark_interpolators(kx_grid, ky_grid, sigma_grid, lattice, n_calls=10000):
    ...
    print(
        f"  Original (scipy):  {time_original:.4f} s  ({time_original / n_calls * 1e6:.2f} µs/call)"
    )
    print(
        f"  Numba:             {time_numba:.4f} s  ({time_numba / n_calls * 1e6:.2f} µs/call)"
    )
